...
class Schedule {
  ...
  addCourseSection (courseSection) {
    if (!courseSection) {
      ...
      // TODO!!
      // Need to consider if two additions are adjacent
      // and increment the index of the later session
  ...
  /**
   * TESTING: adds the first section
   * @param {Course} course 
  ...
  /**
   * TESTING: removes the first section
   * @param {Course} course 
   */
  removeCourse (course) {
    if (!course) {
 ...
 * @param {string} time 
 * @returns {number|null} number of minutes
 */
